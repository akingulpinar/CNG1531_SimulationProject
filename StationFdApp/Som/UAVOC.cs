...
namespace UAVProject.Som
{
  public class CUAVOC : HlaObjectClass
  {
    ...
    public CUAVOC() : base()
    {
      ...
      // LeftBattery
      LeftBattery = new HlaAttribute("LeftBattery", PSKind.Subscribe);
      Attributes.Add(LeftBattery);
      // Speed
      Speed = new HlaAttribute("Speed", PSKind.Subscribe);
      Attributes.Add(Speed);
      // UAVType
      UAVType = new HlaAttribute("UAVType", PSKind.Subscribe);
      Attributes.Add(UAVType);
      // VictimsFound
      VictimsFound = new HlaAttribute("VictimsFound", PSKind.Subscribe);
      Attributes.Add(VictimsFound);
      // Position
      Position = new HlaAttribute("Position", PSKind.Subscribe);
      Attributes.Add(Position);
      // VictimList
      VictimList = new HlaAttribute("VictimList", PSKind.Subscribe);
      Attributes.Add(VictimList);
      // TargetPosition
      TargetPosition = new HlaAttribute("TargetPosition", PSKind.Subscribe);
      Attributes.Add(TargetPosition);
      // Radius
      Radius = new HlaAttribute("Radius", PSKind.Subscribe);
