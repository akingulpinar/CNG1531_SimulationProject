...
namespace UAVProject.Som
{
  public class CUAVOC : HlaObjectClass
  {
    ...
    public CUAVOC() : base()
    {
      ...
      // Create Attributes
      // UAVName
      UAVName = new HlaAttribute("UAVName", PSKind.PublishSubscribe);
      Attributes.Add(UAVName);
      // LeftBattery
      LeftBattery = new HlaAttribute("LeftBattery", PSKind.PublishSubscribe);
      Attributes.Add(LeftBattery);
      // Speed
      Speed = new HlaAttribute("Speed", PSKind.PublishSubscribe);
      Attributes.Add(Speed);
      // UAVType
      UAVType = new HlaAttribute("UAVType", PSKind.PublishSubscribe);
      Attributes.Add(UAVType);
      // VictimsFound
      VictimsFound = new HlaAttribute("VictimsFound", PSKind.PublishSubscribe);
      Attributes.Add(VictimsFound);
      // Position
      Position = new HlaAttribute("Position", PSKind.PublishSubscribe);
      Attributes.Add(Position);
      // VictimList
      VictimList = new HlaAttribute("VictimList", PSKind.PublishSubscribe);
      Attributes.Add(VictimList);
      // TargetPosition
      TargetPosition = new HlaAttribute("TargetPosition", PSKind.PublishSubscribe);
      Attributes.Add(TargetPosition);
      // Radius
      Radius = new HlaAttribute("Radius", PSKind.PublishSubscribe);
